...
class LaneDataRecorder:
    ...
    def prepare_file(self):
        dir = os.path.dirname(self.lane_file)
...
class VehicleManager():
    ...
    def detect_junction(self, waypoint_list):
        ...
        """
        if not junctionMode:
            if DEFAULT_TOWN == ('Town04' or 'Town06'):
            ...
            if junction_argument:
                self.junctionInSightCounter = LANEPOINTS_NUMBER - 20 + offset
            else:
    ...
    def move_agent(self, vehicle, waypoint_list):
        ...
        vehicle.set_transform(carla.Transform(waypoint_list[0 + 6 * int(isThirdPerson)].transform.location +
                                              waypoint_list[0 + 6 * int(
                                                  isThirdPerson)].transform.get_right_vector() * oscillation * (
                                                          2 / math.pi * math.asin(math.sin(self.deviation_counter))),
                                              carla.Rotation(pitch=waypoint_list[
                                                  0 + 6 * int(isThirdPerson)].transform.rotation.pitch,
                                                             yaw=waypoint_list[0 + 6 * int(
                                                                 isThirdPerson)].transform.rotation.yaw + angle * math.sin(
                                                                 self.deviation_counter),
                                                             roll=waypoint_list[0 + 6 * int(
                                                                 isThirdPerson)].transform.rotation.roll)))
