...
from Preparing_data.scripts.label_loader import LabelLoader
from Preparing_data.gather_data import LOAD_DIR, SAVE_DIR, TRAIN_GT,OVERALL_GT, NUMBER_OF_IMAGES,DEFAULT_TOWN,HEIGHT_SAMPLES, max_files_per_classification
loading_directory = LOAD_DIR
...
class ImageSaver():
    ...
    def execute(self):
        ...
        self.label_loader = LabelLoader(TRAIN_GT)
        self.label_loader2 = LabelLoader(TRAIN_GT, OVERALL_GT)
        self.image_name = 0
        self.image_name_gt = 0

        for i, image_set in enumerate(self.load_imagesets()):
            if(image_set.shape[0] == NUMBER_OF_IMAGES):
    ...
    def save_image_to_disk(self, image_array):
        ...
        save_name = saving_directory + str(self.label_loader2.calculate_folder(HEIGHT_SAMPLES))
        if not "nolanes" in save_name:
            ...
            if files_in_folder < max_files_per_classification:
    ...
    def save_images_with_lanepoints(self, image_array):
        ...
        lanepoints = self.label_loader.load_lanepoints()
        if lanepoints is None:
            print("Brak danych punktów lini. pomijanie..")
            return
        for i, lane in enumerate(lanepoints):
            for lanepoint in lane:
                cv2.circle(image_rgb, (lanepoint[0], lanepoint[1]), 3, self.colormap[i], thickness=1)

        image = Image.fromarray(image_rgb)
        save_name = os.path.join('data/debug/', f'{DEFAULT_TOWN}/{self.image_name_gt:04d}')
